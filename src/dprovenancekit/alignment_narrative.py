...
class AlignmentNarrativeCompiler:
    def compile(self, findings: List[AlignmentFinding]) -> str:
        ...
        for f in findings:
            if f.kind == AlignmentFindingKind.REGRESSION_RISK and f.regression_risk is not None:
        ...
        if removals:
            if len(removals) == 1:
                paragraphs.append(f"One critical validation step ('{removals[0]}') was removed.")
            else:
                paragraphs.append(f"{len(removals)} critical steps were removed (e.g., '{removals[0]}').")

        if additions:
            if len(additions) == 1:
                paragraphs.append(f"A new critical step ('{additions[0]}') was introduced.")
            else:
                paragraphs.append(f"{len(additions)} new critical steps were introduced.")
