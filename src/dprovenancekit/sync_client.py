...
class CloudSyncClient:
    def __init__(self, api_url: str = None, api_key: str = None):
        self.api_url = (
            api_url or os.environ.get("DPROV_API_URL", "https://api.dprovenance.dev")
        ).rstrip("/")
    ...
    def push_run(self, run_id: uuid.UUID, store: SQLiteTraceStore) -> None:
        ...
        try:
            ...
            for row in rows:
                edges_data.append(
                    {"source_id": row[0], "target_id": row[1], "type": row[2]}
                )
        except Exception:
    ...
    def pull_run(self, run_id: uuid.UUID, store: SQLiteTraceStore) -> None:
        ...
        for ev_data in resp.get("events", []):
            ...
            type_id = payload_data.get(
                "type", payload_data.get("type_identifier_value", "unknown")
            )
            priority = payload_data.get(
                "priority", payload_data.get("priority_value", 1)
            )
