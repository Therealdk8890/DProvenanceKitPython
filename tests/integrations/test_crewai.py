...
def _kickoff_events(crew, agent, task):
    ...
    return [
        ("crew", "start", crew, FakeEvent(event_id="c", parent_event_id=None, emission_sequence=1, crew=crew, crew_name="research-crew", inputs={"topic": "ai"})),
        ("task", "start", task, FakeEvent(event_id="t", parent_event_id="c", emission_sequence=2, task=task, task_name="research", agent_role=None)),
        ("agent", "start", agent, FakeEvent(event_id="a", parent_event_id="t", emission_sequence=3, agent=agent, agent_role="Researcher", tools=["search"], task_prompt="Summarize AI")),
        ("llm", "start", agent, FakeEvent(event_id="l1", parent_event_id="a", emission_sequence=4, model="gpt-4o", agent_role="Researcher", messages=[{"role": "user", "content": "hi"}], tools=[])),
        ("llm", "end", agent, FakeEvent(event_id="l2", parent_event_id="a", started_event_id="l1", emission_sequence=5, model="gpt-4o", agent_role="Researcher", response="ANSWER", usage={"prompt_tokens": 10, "completion_tokens": 5, "total_tokens": 15})),
        ("tool", "start", None, FakeEvent(event_id="o1", parent_event_id="a", emission_sequence=6, tool_name="search", agent_role="Researcher", tool_args={"q": "ai"})),
        ("tool", "end", None, FakeEvent(event_id="o2", parent_event_id="a", started_event_id="o1", emission_sequence=7, tool_name="search", agent_role="Researcher", output="3 results", from_cache=False)),
        ("agent", "end", agent, FakeEvent(event_id="a2", parent_event_id="t", started_event_id="a", emission_sequence=8, agent=agent, agent_role="Researcher", output="answer")),
        ("task", "end", task, FakeEvent(event_id="t2", parent_event_id="c", started_event_id="t", emission_sequence=9, task=task, task_name="research", agent_role="Researcher", output="answer")),
        ("crew", "end", crew, FakeEvent(event_id="c2", parent_event_id=None, started_event_id="c", emission_sequence=10, crew=crew, crew_name="research-crew", output="final report", total_tokens=42)),
    ]
...
def test_event_roundtrip_and_canonical_encoding():
    ev = CrewAITraceEvent.make("tool.end", TracePriority.STRUCTURAL, {"tool": "search", "output": "r"})
    assert ev.type_identifier == "tool.end"
    assert ev.priority is TracePriority.STRUCTURAL
    assert ev.encode().decode() == '{"output": "r", "priority": 2, "tool": "search", "type": "tool.end"}'
...
def test_records_full_kickoff():
    ...
    assert [e.payload.type_identifier for e in run.events] == ["crew.start", "task.start", "agent.start", "llm.start", "llm.end", "tool.start", "tool.end", "agent.end", "task.end", "crew.end"]
    by_type = _events_by_type(run)
    assert by_type["crew.start"].engine_name == "research-crew"
    assert by_type["task.start"].engine_name == "Researcher"
...
def test_out_of_order_delivery_is_reordered_by_emission_sequence():
    ...
    scrambled = [events[0], events[1], events[2], events[4], events[3], events[6], events[5], events[9], events[7], events[8]]
    run_id = None
...
def test_lifecycle_edges_link_completion_to_start_and_child_to_parent():
    ...
    llm_incoming = {(e.source_id, e.type) for e in store.lineage_edges(by_type["llm.end"].id)}
    assert (by_type["llm.start"].id, TraceEdgeType.DERIVED_FROM) in llm_incoming
    for parent, child in (("crew", "task"), ("task", "agent"), ("agent", "llm"), ("agent", "tool")):
        incoming = {(e.source_id, e.type) for e in store.lineage_edges(by_type[f"{child}.start"].id)}
...
def test_error_event_is_recorded_as_critical():
    ...
    listener = DProvenanceKitEventListener(store)
    crew = FakeCrew("research-crew")
    listener.handle("crew", "start", crew, FakeEvent(event_id="c", emission_sequence=1, crew=crew, crew_name="research-crew", inputs={}))
    listener.handle("tool", "error", None, FakeEvent(event_id="e1", parent_event_id="c", emission_sequence=2, tool_name="search", agent_role="Researcher", error="tool exploded"))
    run_id = listener.run_id_for(crew)
    listener.handle("crew", "error", crew, FakeEvent(event_id="c2", started_event_id="c", emission_sequence=3, crew=crew, crew_name="research-crew", error="crew failed"))
...
def test_sub_event_without_a_run_is_ignored():
    store = InMemoryTraceStore()
    listener = DProvenanceKitEventListener(store)
    listener.handle("llm", "start", None, FakeEvent(event_id="l1", emission_sequence=1, model="gpt-4o"))
