...
def _event(payload, seq):
    return TraceEvent(
        run_id=uuid.uuid4(), context_id="1", engine_name="test", schema_version=1,
        sequence=seq, span_id=None, parent_span_id=None, payload=payload,
    )


def test_write_amplification_defense():
    config = OfflineConfig(
        capacity=BufferCapacity(max_items=1000, max_bytes=1_000_000, max_event_size_bytes=500_000),
    )
    ...
    store = CloudTraceStore(
        ChaosEvent, "https://api.dprovenance.cloud", "test", config=config, transport=transport, start_writer=False
    )
...
def test_poison_batch_quarantine():
    ...
    store = CloudTraceStore(
        ChaosEvent, "https://api.dprovenance.cloud", "test", transport=transport, start_writer=False
    )
    store.record(_event(ChaosEvent.tiny(), 1))
    store.flush()
...
def test_flush_times_out_on_sustained_outage_instead_of_hanging():
    ...
    buffer.enqueue(
        TraceEventRow(
            id=str(uuid.uuid4()), run_id=str(uuid.uuid4()), context_id="1",
            priority=int(TracePriority.STRUCTURAL), sequence=1, engine="test",
            span_id=None, parent_span_id=None, type="chaos", payload=b"x", timestamp=0,
        )
    )
    ...
    writer = CloudWriter("https://api.dprovenance.cloud/ingest", "test", buffer, transport=transport)
