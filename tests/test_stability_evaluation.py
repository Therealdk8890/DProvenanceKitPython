...
def _tool(name, seq):
    return TraceEvent(
        run_id=uuid.uuid4(), context_id="t", engine_name="Agent", schema_version=1,
        sequence=seq, span_id="s", parent_span_id=None, payload=AgentEvent.tool_execution(name, ""),
    )
...
def _single_case_dataset():
    case = BenchmarkCase(
        ...
        description="",
        base_run=TraceRun(run_id=uuid.uuid4(), context_id="t", events=[_tool("Search", 0)]),
        comparison_run=TraceRun(run_id=uuid.uuid4(), context_id="t", events=[_tool("Lookup", 0)]),
        expected_findings=[ExpectedFinding(AlignmentFinding.semantic_evolution("tool", "tool"))],
    )
...
def test_deterministic_engine_has_zero_variance():
    ...
    def factory(_ctx, cb):
        evaluator = AnyEquivalenceEvaluator(
            ...
            evaluator=lambda b, c: 0.95 if (b.type_identifier == "tool" and c.type_identifier == "tool") else 0.0,
        )
        config = AlignmentConfiguration(AlignmentProfile.developer_debug_v1, evaluator)
        return TraceAlignmentEngine(config, capture_mode=VerificationCaptureMode.EVIDENCE_ONLY, meta_trace_callback=cb)

    stability = runner.run_repeated_evaluation(_single_case_dataset(), iterations=3, engine_factory=factory, boundary=boundary)
...
def test_non_deterministic_engine_produces_detectable_variance():
    ...
    def factory(ctx, cb):
        ...
        evaluator = AnyEquivalenceEvaluator(
            ...
            evaluator=lambda b, c, ms=match_score: ms if (b.type_identifier == "tool" and c.type_identifier == "tool") else 0.0,
        )
        config = AlignmentConfiguration(AlignmentProfile.developer_debug_v1, evaluator)
        return TraceAlignmentEngine(config, capture_mode=VerificationCaptureMode.EVIDENCE_ONLY, meta_trace_callback=cb)
...
def test_perturbation_is_gated_by_boundary():
    base = AnyEquivalenceEvaluator(evaluator_identifier="base", evaluator=lambda a, b: 0.9)
